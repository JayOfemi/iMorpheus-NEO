                      ...
                      CValidationState& state, unsigned flags)
{
  ...
  /* Check that no name inputs/outputs are present for a non-Imcoin tx.
     If that's the case, all is fine.  For a Imcoin tx instead, there
     should be at least an output (for NAME_NEW, no inputs are expected).  */

  if (!tx.IsImcoin ())
    {
      if (nameIn != -1)
        return state.Invalid (error ("%s: non-Imcoin tx %s has name inputs",
                                     __func__, txid));
      if (nameOut != -1)
        return state.Invalid (error ("%s: non-Imcoin tx %s at height %u"
                                     " has name outputs",
  ...
  assert (tx.IsImcoin ());
  if (nameOut == -1)
    return state.Invalid (error ("%s: Imcoin tx %s has no name outputs",
                                 __func__, txid));

  /* Reject "greedy names".  */
  const Consensus::Params& params = Params ().GetConsensus ();
  if (tx.vout[nameOut].nValue < params.rules->MinImcoinAmount(nHeight))
                      ...
                      CCoinsViewCache& view, CBlockUndo& undo)
{
  ...
  /* This check must be done *after* the historic bug fixing above!  Some
     ...
     transactions *not* marked as Imcoin tx.  */
  if (!tx.IsImcoin ())
    return;
