                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    // transactions.
    if (fRequireStandard && !tx.IsImcoin() && tx.nVersion >= 2 && VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV) != THRESHOLD_ACTIVE) {
...

    {
        ...
        //
        // Imcoin actually allows some scripts into the mempool that would
...
bool CheckProofOfWork(const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    if (!CheckProofOfWork(block.GetHash(), block.nBits, params))
        return error("%s : check proof of work failed", __func__);
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck> *pvChecks)
{
    ...
    /* Do this as very first action.  Otherwise, we can run into troubles
       ...
       for Imcoin here ensures that the caching is not used for
       Imcoin-specific stuff.  */
    /* TODO: Implement correct caching also for the Imcoin checks,
       if this is deemed beneficial at some point in the future.  */
    if (!CheckNameTransaction (tx, nSpendHeight, inputs, state, flags))
        return error ("CheckInputs: tx invalid for Imcoin");
...
/* Temporary check that blocks are compatible with BDB's 10,000 lock limit.
   ...
   In addition, each Imcoin transaction could touch at most one name,
   so add them as well.  */
